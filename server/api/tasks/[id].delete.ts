...
import { deleteTaskRecord } from "~~/server/lib/services/taskService";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    return await deleteTaskRecord(taskId, getEventDb(event));
  } catch (error) {
    ...
    return sendApiError(event, error, "The task could not be deleted.");
  }
});
